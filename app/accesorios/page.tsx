...
export default function AccesoriosPage() {
    ...
    useEffect(() => {
        ...
        if (categoriasParam) {
            ...
            // Si no hay categorías en la URL, seleccionamos la categoría de accesorios
        }
    ...
    // Aplicar filtro automático para mostrar solo productos de accesorios
    useEffect(() => {
        if (categories.length > 0 && products.length > 0) {
            // Buscar la categoría "Accesorios" entre las categorías disponibles
            const accesoriosCategory = categories.find(cat => 
                cat.parent_nombre.toLowerCase() === 'accesorios' || 
                cat.children.some(child => child.nombre.toLowerCase() === 'accesorios')
            );

            if (accesoriosCategory) {
                // Seleccionar automáticamente la categoría de accesorios
                const accesoriosChildIds = accesoriosCategory.children.map(child => child.id_categoria);
                const newSelectedCategories = new Set(accesoriosChildIds);
                
                setSelectedCategories(newSelectedCategories);

                // Actualizar la URL para reflejar el filtro de accesorios
    ...
    const clearAllFilters = () => {
        // Reset all filters
        // Mantener la categoría de accesorios seleccionada
        const accesoriosCategory = categories.find(cat => 
            cat.parent_nombre.toLowerCase() === 'accesorios' || 
            cat.children.some(child => child.nombre.toLowerCase() === 'accesorios')
        );
        ...
        if (accesoriosCategory) {
            const accesoriosChildIds = accesoriosCategory.children.map(child => child.id_categoria);
            setSelectedCategories(new Set(accesoriosChildIds));
        } else {
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex flex-1">
                            <div className="flex items-center ms-3">
                                <div>
                                    <Input
                                        placeholder="Buscar productos de accesorios..."
                                        value={searchQuery}
                                    ...
                                    <li key={category.parent_id}>
                                        <div className="flex items-center mb-2">
                                            <input
                                                ...
                                                disabled={category.parent_nombre.toLowerCase() === 'accesorios'} // Deshabilitar la casilla de accesorios
                                            />
                                            <label
                                                htmlFor={`parent-${category.parent_id}`}
                                                className={`ms-2 text-sm font-medium ${category.parent_nombre.toLowerCase() === 'accesorios' ? 'text-primary font-bold' : 'text-gray-900 dark:text-gray-300'}`}
                                            >
                                            ...
                                            {category.children.map((child) => (
                                                <li key={child.id_categoria} className="flex items-center">
                                                    <input
                                                        ...
                                                        disabled={category.parent_nombre.toLowerCase() === 'accesorios'} // Deshabilitar las subcategorías de accesorios
                                                    />
                                                    <label
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        {filteredProducts.length > 0 ? (
                            ...
                            <div className="col-span-full text-center py-10">
                                <p className="text-gray-500">No se encontraron productos de accesorios</p>
                            </div>
                        )}
                    </div>
                </div>
