...
export default function RopaPage() {
    ...
    useEffect(() => {
        ...
        if (categoriasParam) {
            ...
            // Si no hay categorías en la URL, seleccionamos la categoría de ropa
            // Aquí necesitamos saber el ID específico de la categoría "Ropa"
            // Por ahora, lo dejaremos vacío y lo configuraremos después de cargar las categorías
        }
        ...
        const getProductos = async () => {
            ...
            // Filtrar productos por la categoría de ropa
            // Aquí necesitamos identificar el ID de la categoría "Ropa"
            // Suponiendo que sabemos que el ID de "Ropa" es un valor específico
            // Buscaremos el ID de la categoría "Ropa" en las categorías obtenidas
        }

        getProductos();
    }, []);

    // Aplicar filtro automático para mostrar solo productos de ropa
    useEffect(() => {
        if (categories.length > 0 && products.length > 0) {
            // Buscar la categoría "Ropa" entre las categorías disponibles
            const ropaCategory = categories.find(cat => 
                cat.parent_nombre.toLowerCase() === 'ropa' || 
                cat.children.some(child => child.nombre.toLowerCase() === 'ropa')
            );

            if (ropaCategory) {
                // Seleccionar automáticamente la categoría de ropa
                const ropaChildIds = ropaCategory.children.map(child => child.id_categoria);
                const newSelectedCategories = new Set(ropaChildIds);
                
                // Asegurar que también se incluya el padre si tiene hijos
                setSelectedCategories(newSelectedCategories);

                // Actualizar la URL para reflejar el filtro de ropa
    ...
    const clearAllFilters = () => {
        // Reset all filters
        // Mantener la categoría de ropa seleccionada
        const ropaCategory = categories.find(cat => 
            cat.parent_nombre.toLowerCase() === 'ropa' || 
            cat.children.some(child => child.nombre.toLowerCase() === 'ropa')
        );
        ...
        if (ropaCategory) {
            const ropaChildIds = ropaCategory.children.map(child => child.id_categoria);
            setSelectedCategories(new Set(ropaChildIds));
        } else {
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex flex-1">
                            <div className="flex items-center ms-3">
                                <div>
                                    <Input
                                        placeholder="Buscar productos de ropa..."
                                        value={searchQuery}
                                    ...
                                    <li key={category.parent_id}>
                                        <div className="flex items-center mb-2">
                                            <input
                                                ...
                                                disabled={category.parent_nombre.toLowerCase() === 'ropa'} // Deshabilitar la casilla de ropa
                                            />
                                            <label
                                                htmlFor={`parent-${category.parent_id}`}
                                                className={`ms-2 text-sm font-medium ${category.parent_nombre.toLowerCase() === 'ropa' ? 'text-primary font-bold' : 'text-gray-900 dark:text-gray-300'}`}
                                            >
                                            ...
                                            {category.children.map((child) => (
                                                <li key={child.id_categoria} className="flex items-center">
                                                    <input
                                                        ...
                                                        disabled={category.parent_nombre.toLowerCase() === 'ropa'} // Deshabilitar las subcategorías de ropa
                                                    />
                                                    <label
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        {filteredProducts.length > 0 ? (
                            ...
                            <div className="col-span-full text-center py-10">
                                <p className="text-gray-500">No se encontraron productos de ropa</p>
                            </div>
                        )}
                    </div>
                </div>
