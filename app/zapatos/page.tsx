...
export default function ZapatosPage() {
    ...
    useEffect(() => {
        ...
        if (categoriasParam) {
            ...
            // Si no hay categorías en la URL, seleccionamos la categoría de zapatos
        }
    ...
    // Aplicar filtro automático para mostrar solo productos de zapatos
    useEffect(() => {
        if (categories.length > 0 && products.length > 0) {
            // Buscar la categoría "Zapatos" entre las categorías disponibles
            const zapatosCategory = categories.find(cat => 
                cat.parent_nombre.toLowerCase() === 'zapatos' || 
                cat.children.some(child => child.nombre.toLowerCase() === 'zapatos')
            );

            if (zapatosCategory) {
                // Seleccionar automáticamente la categoría de zapatos
                const zapatosChildIds = zapatosCategory.children.map(child => child.id_categoria);
                const newSelectedCategories = new Set(zapatosChildIds);
                
                setSelectedCategories(newSelectedCategories);

                // Actualizar la URL para reflejar el filtro de zapatos
    ...
    const clearAllFilters = () => {
        // Reset all filters
        // Mantener la categoría de zapatos seleccionada
        const zapatosCategory = categories.find(cat => 
            cat.parent_nombre.toLowerCase() === 'zapatos' || 
            cat.children.some(child => child.nombre.toLowerCase() === 'zapatos')
        );
        ...
        if (zapatosCategory) {
            const zapatosChildIds = zapatosCategory.children.map(child => child.id_categoria);
            setSelectedCategories(new Set(zapatosChildIds));
        } else {
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex flex-1">
                            <div className="flex items-center ms-3">
                                <div>
                                    <Input
                                        placeholder="Buscar productos de zapatos..."
                                        value={searchQuery}
                                    ...
                                    <li key={category.parent_id}>
                                        <div className="flex items-center mb-2">
                                            <input
                                                ...
                                                disabled={category.parent_nombre.toLowerCase() === 'zapatos'} // Deshabilitar la casilla de zapatos
                                            />
                                            <label
                                                htmlFor={`parent-${category.parent_id}`}
                                                className={`ms-2 text-sm font-medium ${category.parent_nombre.toLowerCase() === 'zapatos' ? 'text-primary font-bold' : 'text-gray-900 dark:text-gray-300'}`}
                                            >
                                            ...
                                            {category.children.map((child) => (
                                                <li key={child.id_categoria} className="flex items-center">
                                                    <input
                                                        ...
                                                        disabled={category.parent_nombre.toLowerCase() === 'zapatos'} // Deshabilitar las subcategorías de zapatos
                                                    />
                                                    <label
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        {filteredProducts.length > 0 ? (
                            ...
                            <div className="col-span-full text-center py-10">
                                <p className="text-gray-500">No se encontraron productos de zapatos</p>
                            </div>
                        )}
                    </div>
                </div>
